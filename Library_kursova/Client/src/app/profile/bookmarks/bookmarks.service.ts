...
export class BookmarksService {
  ...
  getBookmarksBook(email: string) {
    return this.http.get<Book[]>(
      'https://localhost:5001/api/bookmarkbook/bookmarks/?email=' + email
    );
  }

  isBookInBookmarks(bookId: number, email: string) {
    return this.http
      .get<boolean>(
        'https://localhost:5001/api/bookmarkbook/bookmark/?bookId=' +
          bookId +
          '&email=' +
          email
      )
  ...
  addBookmarkBook(bookmarkBookDTO: { bookId: number; email: string }) {
  ...
  deleteBookmarkBook(bookmarkBookDTO: { bookId: number; email: string }) {
    return this.http
      .delete(
