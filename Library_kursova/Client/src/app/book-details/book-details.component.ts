...
export class BookDetailsComponent implements OnInit, OnChanges {
  ...
  onCreateComment(form: NgForm) {
    ...
    this.commentDTO.email = this.userDTO.email;
  ...
  onAddBookmark() {
    if (this.isBookmark === false) {
      this.bookmarkService
        .addBookmarkBook({ bookId: this.id, email: this.userDTO.email })
        .subscribe({
          next: () => {
            this.toastr.success(
      ...
      this.bookmarkService
        .deleteBookmarkBook({ bookId: this.id, email: this.userDTO.email })
        .subscribe({
          next: () => {
            this.toastr.success(
  ...
  onAddLike() {
    if (this.isLiked === false) {
      this.bookDetailService
        .addLike({ bookId: this.id, email: this.userDTO.email })
        .subscribe({
          next: () => {
            this.toastr.success(
      ...
      this.bookDetailService
        .deleteLike({ bookId: this.id, email: this.userDTO.email })
        .subscribe({
          next: () => {
            this.toastr.success(
